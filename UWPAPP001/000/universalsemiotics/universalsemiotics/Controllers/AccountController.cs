...
namespace universalsemiotics.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                if (resultCaptcha.ErrorCodes != null)
                {
                ...
                {
                    ModelState.AddModelError("", Resources.universalsemiotics.recaptcha_invalid);
                }
                return View(model);
            }
            ...
            ApplicationUser appUser = await UserManager.FindByEmailAsync(model.Email.ToLower());

            if (appUser != null)
            {
                bool emailStatus = await UserManager.IsEmailConfirmedAsync(appUser.Id);
                if (emailStatus == false)
                {
                    ModelState.AddModelError(nameof(model.Email), Resources.universalsemiotics.email_no_confirmado);
                    return View(model);
                }
            }
            ...
            switch (result)
            {
                ...
                default:
                    ModelState.AddModelError("", "Intento de inicio de sesión no válido / invalid Login");
                    return View(model);
            }
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    String body = "Para activar su cuenta, haga clic <a target='_blank' href=\"" + callbackUrl + "\">aquí</a><br/>"
                        + "To activate your account, click <a target='_blank' href=\"" + callbackUrl + "\">here</a>";

                    Utils.SendMail(model.Email.ToLower(), "Active / Activate - universalsemiotics", body);

                    String controlador = idioma == "es-ES" ? "es" :
                            idioma == "en-US" ? "en" :
                            "es";
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                String body = "Para reestablecer su contraseña, haga clic <a target='_blank' href=\"" + callbackUrl + "\">aquí</a><br/>"
                    + "To reset your password, click <a target='_blank' href=\"" + callbackUrl + "\">here</a>";

                Utils.SendMail(model.Email.ToLower(), "Reestablecer / Reset - universalsemiotics", body);
